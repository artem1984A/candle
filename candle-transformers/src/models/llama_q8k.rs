...
fn load_perm_sidecar(path_q8k: &Path) -> Result<Option<Vec<usize>>> {
    ...
    let bytes = fs::read(&p)?;
    if bytes.len() < 8 {
        bail!("perm too small: {}", p.display());
    }
    let magic = u32::from_le_bytes(bytes[0..4].try_into().unwrap());
    if magic != 0x4D52_4550 {
        bail!("bad perm magic: {}", p.display());
    }
    let k = u32::from_le_bytes(bytes[4..8].try_into().unwrap()) as usize;
    if bytes.len() != 8 + 4 * k {
        bail!("perm size mismatch: {}", p.display());
    }
...
fn read_q8k(path: &Path) -> Result<(Vec<BlockQ8K>, usize, usize)> {
    ...
    if hdr.dtype != DTYPE_Q8K_TAG {
        bail!("unexpected dtype tag in {}", path.display());
    }
...
impl QuantLinearQ8K {
    ...
    fn forward_2d(&self, x2d: &Tensor) -> Result<Tensor> {
        ...
        let mut lhs: Vec<f32> = x_f32.to_vec1()?;
        if let Some(p) = &self.perm {
            // permute each row of lhs by p: out[j] = in[perm[j]]
            for bi in 0..b {
                let row = &lhs[bi * self.k..(bi + 1) * self.k].to_vec();
                for (j, &src) in p.iter().enumerate() {
                    lhs[bi * self.k + j] = row[src];
                }
            }
        }
...
impl Layer {
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        let y = att.matmul(&v)?.transpose(1, 2)?.reshape((b, s, nh * hd))?;
...
// Build model: .q8k for linears, safetensors for embedding + norms
pub fn build_model(
    ...
    Ok(TinyLlamaQ8K {
        embed: Tensor::zeros((1,), DType::F32, device)?,
        embed_weight,
        layers,
        norm_w,
        lm_head,
